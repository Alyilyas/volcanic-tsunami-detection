...
All figures are generated by the *same script*: `code/generate_spectral_detection_plot.py`.
...
### ➡️ To reproduce figure 11 (Non-overlapping windows)
This figure establishes the detection threshold using non-overlapping (200-sample) windows for station `SBJI`.
Set the `CONFIG` dictionary as follows:
...
"step": 200,              # Set to 200 for non-overlapping windows
"station": 'SBJI',
...
### ➡️ To reproduce figure 12 (1-second overlapping windows)
This figure establishes the detection threshold using 1-second overlapping windows for station `SBJI`.
Set the `CONFIG` dictionary as follows:
```python
...
"step": 20,              # Set to 20 for 1-second overlapping windows
"station": 'SBJI',
...
```
### ➡️ To reproduce supplementary information S4 figures (other stations/components)
The figures in supplementary information (e.g., Fig. S10-21) are generated by running the same analysis as Figure 11, but on different stations or components.
Example: To reproduce the plot for station `CGJI` (non-overlapping):
```python
CONFIG = {
    "station": 'CGJI', # <-- Changed station
    "component": 'BHE',
    "step": 200,
    "n_bootstrap": 10000,
    "ar_lag": 11, # <-- Note: AR lag may change per station (see Table 1)
    "window_size": 200,
    "t_step": 10,
    # ... (other parameters)
}
```
Simply change the `"station"` and `"component"` values (and verify `"ar_lag"` against Table 1) to generate the remaining plots for the appendix.
...
|---------|-----------------------------|--------|
| **Figure 11** | <pre><code>"station": 'SBJI'<br>"component": 'BHE'<br>"step": 200<br>"n_bootstrap": 10000<br>"ar_lag": 10</code></pre> | Non-overlapping windows. |
| **Figure 12** | <pre><code>"station": 'SBJI'<br>"component": 'BHE'<br>"step": 20<br>"n_bootstrap": 10000<br>"ar_lag": 10</code></pre> | 1-second overlapping windows. |
| **Appendix B** | <pre><code>"station": 'CGJI' (or other)<br>"component": 'BHE' (or other)<br>"step": 200<br>"n_bootstrap": 10000<br>"ar_lag": 11 (or other)</code></pre> | Repeat for all station/component combinations in the Appendix. Crucially, check *Manuscript Table 1* for the correct `ar_lag` for each station. |
| **Quick Test** | <pre><code>"n_bootstrap": 100</code></pre> | Use this setting for any figure to run a fast, low-quality test for debugging. |
